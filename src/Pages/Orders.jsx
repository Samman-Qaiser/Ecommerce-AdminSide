              ...
              <TableBody>
                {orders.map((order) => {
                  ...
                  const currentDisplayStatus =
                    pendingChanges[order.id] || order.status;
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${order.payment === "Paid" ? "border-green-200 text-green-600 bg-green-50" : "border-red-200 text-red-600 bg-red-50"}`}
                        >
                          {order.payment}
                        </span>
                      </TableCell>
                      <TableCell>
                        <Select
                          onValueChange={(val) =>
                            handleStatusSelect(order.id, val)
                          }
                          defaultValue={order.status}
                        >
                          <SelectTrigger
                            className={`w-32.5 h-9 text-xs font-bold rounded-lg border shadow-none                 ${getStatusColor(currentDisplayStatus)}`}
                          >
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <TableCell className="text-right pr-6">
                        {isDirty ? (
                          <Button
                            ...
                            className="bg-primary hover:bg-primary/50 text-white rounded-lg px-4 transition-all animate-in fade-in zoom-in duration-200"
                          >
                            {loadingId === order.id ? (
